...
def A_Star_Search(V, s, g, Number, distance):
    ...
    CostTo.update({s:0})
    EstTotalCost.update({s:distance(s,g)})
    ...
    while len(Q)>0:
        ...
        for i in Number(v):
            pvi = CostTo[v] + distance(v,i)

            if pvi < CostTo[i]:
                pred.update({i:v})
                CostTo.update({i:pvi})
                EstTotalCost.update({i: pvi + distance(i,g)})
...
if __name__ == '__main__':
    ...
    opt_path = A_Star_Search(occupancy_grid,(635, 140),(350, 400),Number,distance)
